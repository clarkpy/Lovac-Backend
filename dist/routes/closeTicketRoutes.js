...
router.post("/close-ticket", async (req, res) => {
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(0xff0000);
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`acceptClose_${ticketId}`).setLabel("Accept & Close").setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`denyClose_${ticketId}`).setLabel("Keep Open").setStyle(ButtonStyle.Danger));
...
router.post("/force-close-ticket", async (req, res) => {
    try {
